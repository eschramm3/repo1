...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void decrease(int loc) {
		...
		while(loc > 1 && array[parentLoc].getValue().compareTo(array[loc].getValue()) > 0){
			Decreaser<T> holder = array[parentLoc];
			array[parentLoc] = array[loc];
...
//			holder.loc = loc;
	...
	private void heapify(int where) {
		...
		//
		int childL = where*2;
		int childR = (where*2)+1;
		int baby;
		if(childL <= this.size){
			if(childR <= this.size){
				if(array[childR].getValue().compareTo(array[childL].getValue()) >= 0) //which is the baby of the family
					baby = childL;
				else
					baby = childR;
				if(array[baby].getValue().compareTo(array[where].getValue()) < 0){
					decrease(baby);
					heapify(baby);
				}
			}
			else {
				if(array[childL].getValue().compareTo(array[where].getValue()) < 0){
					decrease(childL);
					heapify(childL);
				}
			}
